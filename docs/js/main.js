...
function who_won(game_state) {
  if (game_state[0][0] != EMPTY) {
    if (
      ...
      game_state[0][0] == game_state[0][2]
    ) {
      return game_state[0][0];
    }
    if (
      game_state[0][0] == game_state[1][0] &&
      game_state[0][0] == game_state[2][0]
    ) {
      return game_state[0][0];
    }
    if (
  ...
  if (game_state[2][2] != EMPTY) {
    if (
      game_state[2][2] == game_state[2][1] &&
      game_state[2][2] == game_state[2][0]
    ) {
      return game_state[2][2];
    }
    if (
      game_state[2][2] == game_state[1][2] &&
      game_state[2][2] == game_state[0][2]
    ) {
      return game_state[2][2];
    }
  }
  if (
    game_state.every(function (arr) {
  ...
  for (row_id = 0; row_id < 3; row_id++) {
    for (col_id = 0; col_id < 3; col_id++) {
      ...
      switch (game_state[row_id][col_id]) {
        case PLAYER_1:
          $(div_id).addClass("nemo");
          break;
        case PLAYER_0:
          $(div_id).addClass("dolly");
          break;
        default:
          break;
      }
...
function reset_game(e) {
  $("#status").html("");
  $("#status").removeClass("nemo");
  $("#status").removeClass("dolly");
  $(".cell").removeClass("nemo");
  $(".cell").removeClass("dolly");
  game_state = [
    [EMPTY, EMPTY, EMPTY],
    [EMPTY, EMPTY, EMPTY],
    [EMPTY, EMPTY, EMPTY],
...
async function update(element) {
  ...
  // console.log(cls);
  if (cls.includes("nemo") || cls.includes("dolly")) {
    return;
  }
  update_html(game_state);
  result_val = who_won(game_state);
  switch (result_val) {
    case PLAYER_1:
      $("#status").html("Won.");
      $("#status").addClass("nemo");
      break;
    case PLAYER_0:
      $("#status").html("Won.");
      $("#status").addClass("dolly");
      break;
    case EMPTY:
      $("#status").html("Ongoing...");
      break;
